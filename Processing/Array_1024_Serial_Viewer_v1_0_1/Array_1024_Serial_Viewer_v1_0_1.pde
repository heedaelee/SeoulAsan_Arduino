...
byte[] inBytes = new byte[NUM_SENSOR*2 + 4];
  ...
  try {
    ...
    for(int i=0; i<NUM_ROW; i++){
      for(int j=0; j<NUM_COLUMN; j++){
        ...
        data[i][j] = byte2int(inBytes[(i*NUM_COLUMN+j)*2+4],inBytes[(i*NUM_COLUMN+j)*2+3]); //byte to int
        println("after byte2int : "+data[i][j]);
...
int byte2int(byte msb, byte lsb){ 
    int i = 0;
    i = msb << 8;
    i |= lsb & 0xFF; //bit or calculating, then assignment
  return i;    
}
